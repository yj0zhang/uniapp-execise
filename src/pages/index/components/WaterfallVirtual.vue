	...
	export default {
		...
		methods: {
			...
			getColumnNum() {
				...
				const boxWidth = this.containerWidth = this.$refs.container.$el.offsetWidth;
			...
			getColumnList(){
				const {columnNum, containerWidth} = this;
			...
			updateImagePositionInfo(list, startRenderIdx = 0) {
				...
				for(let i = 0; i < list.length; i ++) {
					...
					const params = {
						...
						height: list[i].height * (this.columnWidth / list[i].width),
					}
			...
			setContainerHeight() {
				...
				if(this.$refs.container) {
					this.$refs.container.$el.style.height = this.columnList[this.columnList.length - 1].columnHeight + CONTAINER_PADDING_BOTTOM + 'px';
				}
			},
			renderImage(startBy = 0) {
				const container = this.$refs.container?.$el;
				if(this.imagePositionInfo.length === 0 || !container) {
					return;
				}
				const tempRenderMap = {};
				for(let i = startBy; i < this.imagePositionInfo.length; i ++) {
					...
					const dom = container.querySelector(`.item_${idx}`)
					...
					if(dom) {
						...
						container.appendChild(tempRenderMap[idx])
					}
			...
			createDom(domNode, info) {
				...
				const { img, width, height, left, top, idx } = info;
				imgEl.src = img;
			...
			scrollEventRegistry() {
				const container = this.$refs.outerContainer
				if(!container) {
					return;
				}
				const handleScroll = throttle(async () => {
					const curScrollTop = container.$el.scrollTop;
					...
					if(curScrollTop + container.$el.offsetHeight >= container.$el.scrollHeight * 0.85) {
